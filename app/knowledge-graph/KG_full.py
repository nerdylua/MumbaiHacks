...
class autoKG():
    ...
    def core_text_filter(self, core_list, max_length):
        if self.llm_model.startswith(("gpt-4.1-mini")):
            model = "gpt-4.1-mini"
        else:
            model = "gpt-3.5-turbo-16k"

        header = f"""
You are an advanced AI assistant, specialized in analyzing insurance and mediclaim information \
...
Raw Keywords: 
Health insurance,mediclaim policy,Health insurance policy..,premium payment,cashless treatment,policy coverage.,pre-existing conditions

Processed Keywords: 
Health insurance,mediclaim policy,premium payment,cashless treatment,policy coverage,pre-existing conditions
...
Processed Keywords: 
Health insurance policy,Claim settlement process,Network hospitals,Policy exclusions,Waiting period
"""
        prompt = f"""
{header}        
...
- For keywords that are closely related, expressing different aspects of the same core idea, you need to 'concentrate' them into a single term that best captures the overall concept. For instance, consider the keywords <claim settlement>, <claim processing>, <mediclaim settlement>, <insurance claim process>, <claim reimbursement>. These all relate to the central concept of 'claim settlement' and should be consolidated into this single keyword.
...
For instance, consider a keyword like 'ICICI mediclaim'. Here, 'ICICI' is a distinct entity (insurance company), and 'mediclaim' is a separate concept. Therefore, it's appropriate to split this keyword into two: 'ICICI' and 'mediclaim'.
...
"""
    ...
    def summary_contents(self, indx, sort_inds, avoid_content=None,
        ...
        if model.startswith("gpt-4.1-mini"):
            ...
            raise ValueError("Model should be either GPT-3.5 or GPT-4.1-mini")
