...
COMPLETIONS_MODEL = os.getenv("OPENAI_API_MODEL", "gpt-4.1-mini")
...
def get_completion(prompt, model_name="gpt-4.1-mini", max_tokens=250, retry_times=3, temperature=0.9, top_p=1.0):
    """
    Generate chat completions using the OpenAI API.

    Parameters:
    - prompt: The input prompt for the chat.
    - model: The model to use for generating completions. Default is "gpt-4.1-mini".
...
def my_text_splitter(input_text, chunk_size=200, separator=None, model='gpt-4.1-mini'):
