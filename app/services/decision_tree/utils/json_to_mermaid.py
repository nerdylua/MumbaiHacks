...
class SimpleMermaidGenerator:
    ...
    def generate_mermaid_tree(self, tree: Dict[str, Any]) -> str:
        ...
        # Second pass: generate node definitions
        for node_id, node_data in nodes.items():
            ...
            if node_type == 'root':
                shape = f'{clean_id}[["🏁 {question}"]]'
            elif node_type == 'leaf':
                answer = self.sanitize_text(node_data.get('answer', 'Result'))
                shape = f'{clean_id}(["✅ {answer}"])'
            else:
                shape = f'{clean_id}{{"❓ {question}"}}'
        ...
        lines.extend([
            ...
            "classDef decisionStyle fill:#4ecdc4,stroke:#333,stroke-width:2px,color:#fff", 
            "classDef leafStyle fill:#96ceb4,stroke:#333,stroke-width:2px,color:#000"
        ])
